...
def deserialize_crypt(data_bytes):
    ...
    crypt_list = []
    offset = 4
    chunk_size = 4 * 4 * 8  # 128 bytes per 4x4 uint64 matrix
...
async def main():
    ...
    n = int(input("Nonce / Key 3 (int) [default 1337]: ").strip() or 1337)
    ...
    async def receive():
        while True:
            ...
            try:
                ...
                msg_text = decrypt(crypt_list, k1, k2, n)
            except Exception as e:
                msg_text = f"[Never Gonna Give You Up: {e}]"
    ...
    while True:
        ...
        encrypted_list = encrypt(msg, k1, k2, n)
